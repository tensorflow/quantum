...
def is_gate_approx_eq(gate_true, gate_deser, atol=1e-5):
    ...
    supported_gates = serializer.SERIALIZER.supported_gate_types()
    if not (any([isinstance(gate_true, g) or gate_true is g for g in supported_gates]) and
            any([isinstance(gate_deser, g) or gate_true is g for g in supported_gates])):
