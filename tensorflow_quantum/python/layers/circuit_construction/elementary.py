    ...
    def call(self, inputs, *, append=None, prepend=None):
        ...
        if append is not None:
            ...
            if not tf.is_tensor(append):
                raise TypeError("Append circuits cannot be parsed with given input: {}".format(append))
        ...
        if not tf.is_tensor(prepend):
            raise TypeError("Prepend circuits cannot be parsed with given input: {}".format(prepend))
