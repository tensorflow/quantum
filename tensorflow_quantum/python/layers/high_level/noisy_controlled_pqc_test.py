...
class NoisyControlledPQCTest(tf.test.TestCase, parameterized.TestCase):
    ...
    def test_controlled_pqc_model_circuit_error(self):
        ...
        with self.assertRaisesRegex(TypeError, expected_regex="cirq.Circuit"):
            noisy_controlled_pqc.NoisyControlledPQC(
                'junk', cirq.Z(bit), repetitions=500, sample_based=False)

        with self.assertRaisesRegex(ValueError,
                                    expected_regex="no sympy.Symbols"):
            noisy_controlled_pqc.NoisyControlledPQC(
                no_symbols, cirq.Z(bit), repetitions=500, sample_based=False)
    ...
    def test_controlled_pqc_operators_error(self):
        ...
        with self.assertRaisesRegex(
            ...
            noisy_controlled_pqc.NoisyControlledPQC(
                learnable_flip, 'junk', repetitions=500, sample_based=False)

        with self.assertRaisesRegex(TypeError, expected_regex="Each element"):
            noisy_controlled_pqc.NoisyControlledPQC(
                learnable_flip, [[cirq.Z(bit)]], repetitions=500, sample_based=False)
    ...
    def test_noisy_controlled_pqc_sample_based_error(self):
        ...
        with self.assertRaisesRegex(
            ...
            noisy_controlled_pqc.NoisyControlledPQC(learnable_flip, cirq.Z(qubit), repetitions=10)
    ...
    def test_controlled_pqc_symbols_property(self):
        ...
        test_circuit = cirq.Circuit(
            ...
            cirq.X(bit)**d,
            cirq.Y(bit)**c)
        layer = noisy_controlled_pqc.NoisyControlledPQC(
    ...
    def test_controlled_pqc_simple_learn(self, sample_based):
        ...
        outputs = noisy_controlled_pqc.NoisyControlledPQC(circuit,
                                               cirq.Z(bit),
                                               repetitions=5000,
