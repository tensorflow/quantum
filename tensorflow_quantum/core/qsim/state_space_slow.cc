  ...
  for (uint64_t i = 0; i < sizei; i += 2 * sizej) {
    for (uint64_t j = 0; j < sizej; j += 2 * sizek) {
      for (uint64_t k = 0; k < sizek; k += 2) {
        ...
        p = si;
        data[p + 0] = s0r * m[0] - s0i * m[1] + s1r * m[2] -
                              s1i * m[3] + s2r * m[4] - s2i * m[5] +
                              s3r * m[6] - s3i * m[7];
        data[p + 1] = s0r * m[1] + s0i * m[0] + s1r * m[3] +
                              s1i * m[2] + s2r * m[5] + s2i * m[4] +
                              s3r * m[7] + s3i * m[6];
        p = si | sizek;
        data[p + 0] = s0r * m[8] - s0i * m[9] + s1r * m[10] -
                              s1i * m[11] + s2r * m[12] - s2i * m[13] +
                              s3r * m[14] - s3i * m[15];
        data[p + 1] = s0r * m[9] + s0i * m[8] + s1r * m[11] +
                              s1i * m[10] + s2r * m[13] + s2i * m[12] +
                              s3r * m[15] + s3i * m[14];
        p = si | sizej;
        data[p + 0] = s0r * m[16] - s0i * m[17] + s1r * m[18] -
                              s1i * m[19] + s2r * m[20] - s2i * m[21] +
                              s3r * m[22] - s3i * m[23];
        data[p + 1] = s0r * m[17] + s0i * m[16] + s1r * m[19] +
                              s1i * m[18] + s2r * m[21] + s2i * m[20] +
                              s3r * m[23] + s3i * m[22];
        p |= sizek;
        data[p + 0] = s0r * m[24] - s0i * m[25] + s1r * m[26] -
                              s1i * m[27] + s2r * m[28] - s2i * m[29] +
                              s3r * m[30] - s3i * m[31];
        data[p + 1] = s0r * m[25] + s0i * m[24] + s1r * m[27] +
...
tensorflow::Status StateSpaceSlow::ApplyGate1(const float* matrix) {
  ...
  r_0 = data[0] * matrix[0] - data[1] * matrix[1] +
        data[2] * matrix[2] - data[3] * matrix[3];
  i_0 = data[0] * matrix[1] + data[1] * matrix[0] +
        data[2] * matrix[3] + data[3] * matrix[2];

  r_1 = data[0] * matrix[4] - data[1] * matrix[5] +
        data[2] * matrix[6] - data[3] * matrix[7];
  i_1 = data[0] * matrix[5] + data[1] * matrix[4] +
        data[2] * matrix[7] + data[3] * matrix[6];
...
std::complex<float> StateSpaceSlow::GetAmpl(const uint64_t i) const {
  return std::complex<float>(this->GetRawState()[2 * i], this->GetRawState()[2 * i + 1]);
}
