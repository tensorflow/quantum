...
# Order: explicit env -> python3 (>= 3.11)
PY="${PYTHON_BIN_PATH:-}"
if [[ -z "${PY}" ]]; then
  if ! command -v python3 >/dev/null 2>&1; then
    echo "ERROR: python3 not found. Set PYTHON_BIN_PATH to a Python 3.11+ interpreter." >&2
    exit 2
  fi

  # Require Python >= 3.11 for TFQ.
  if ! python3 - <<'PY'
import sys
sys.exit(0 if sys.version_info[:2] >= (3, 11) else 1)
PY
  then
    echo "ERROR: Python 3.11+ required for TensorFlow Quantum; found $(python3 -V 2>&1)." >&2
    exit 2
  fi
