...
following scripts to check that TensorFlow Quantum continues to run correctly
and code conforms to this project's [coding conventions](#coding-conventions):
...
2.  `./scripts/lint_all.sh`. We use [pylint](https://www.pylint.org/) to
    ensure that code has proper formatting and is lint free.
...
*   For C++ code and Protobuf definitions, we follow the [Google C++ Style
    Guide](https://google.github.io/styleguide/cppguide.html) and [TensorFlow
    specific style
...
*   Represent a single, self-contained change, such as a specific bug fix or
    addition of a specific feature. It should not combine unrelated changes,
...
*   _For new ideas_: If your idea isn't covered by an existing issue, please
    **open a new issue to discuss your proposed changes before starting work**.
    This ensures we agree on the approach before you invest more time.
